...
CREATE TABLE IF NOT EXISTS orders (
    ...
    fulfilled_at Nullable(DateTime64(6)),
    -- CDC 元数据
    _kafka_offset Int64,
...
SELECT
    ...
    currency,
    fromUnixTimestamp64Micro(toInt64(created_at)) AS created_at,
    fromUnixTimestamp64Micro(toInt64(updated_at)) AS updated_at,
    if(notEmpty(cancelled_at), fromUnixTimestamp64Micro(toInt64(cancelled_at)), NULL) AS cancelled_at,
    if(notEmpty(fulfilled_at), fromUnixTimestamp64Micro(toInt64(fulfilled_at)), NULL) AS fulfilled_at,
    0 AS _kafka_offset,
...
CREATE TABLE IF NOT EXISTS customer_stats_mv (
    ...
    first_order_date AggregateFunction(min, DateTime64(6)),
    last_order_date AggregateFunction(max, DateTime64(6)),
